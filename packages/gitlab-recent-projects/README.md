...
- 进入视图后自动读取每个 Fork 的最近分支；仓库仅作为分组分割标题，分支是列表中的主要内容。
- 每个分支右侧提供无边框检查图标。点击后才检查该分支的合入状态；检查中图标会旋转，完成后可重新检查。
...
脚本进入分支视图时读取 10 个 Fork 的分支列表，每个 Fork 最多读取 Branches API 返回的前 100 个分支并展示 5 个活跃业务分支。合入判断仍只检查用户主动选择的分支，同一分支最多并行检查 4 个目标环境。
...
进入分支视图时会请求各 Fork 的分支列表；Upstream 环境分支和 compare API 只在用户点击具体分支的检查按钮后请求。同一业务分支最多并行检查 4 个目标环境。
