...
- 点击面板右上角的分支图标进入「分支状态」。
- Fork 按“收藏优先、最近 MR 项目随后”排列，最多展示 10 个仓库；没有 Upstream 的普通仓库不会进入此视图。
- 每个 Fork 按需点击「检查分支状态」后才读取数据，避免页面后台扫描所有仓库。
- 脚本从 Fork 读取业务分支，并自动对照对应 Upstream 的环境分支。
- GitLab Branches API 返回的分支会按最新提交时间重新排序，只展示近 90 天活跃、且不属于 `dev`、`test`、`blue`、`master` 的前 5 个业务分支。
- 每个业务分支分别显示是否已合入 Upstream 的 `dev`、`test`、`blue`、`master`；目标分支不存在时明确标记为“不存在”。
...
- “最近活跃分支”依据分支最新 commit 的 `committed_date`，近 90 天内有提交才会展示。
