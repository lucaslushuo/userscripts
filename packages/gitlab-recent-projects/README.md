...
- 进入视图后自动读取每个 Fork 的最近分支，并自动检查全部分支的合入状态；仓库仅作为分组分割标题，分支是列表中的主要内容。
- 面板右上角的刷新按钮会重新读取仓库和分支，并重新检查全部合入状态。
- 检查中的目标状态使用无背景的三点明暗波动，不使用临时颜色；状态完成后在固定位置显示红、绿或灰色结果。系统启用“减少动态效果”时，三点保持静态。
- 每个分支使用两行布局：第一行展示完整分支名和 MR 快捷入口，第二行展示提交日期及固定列宽的状态文字。状态结果不使用标签背景或边框，仅通过红、绿、灰文字区分。
- 每个分支右侧提供创建 MR 的快捷入口：当前 Fork 分支作为 source、对应 Upstream 作为 target project，默认目标分支为 `master`。链接会进入 GitLab 的分支确认流程，让 GitLab 先重新计算分支差异，用户仍可修改目标分支后再创建 MR。
...
- 脚本在页面可见时每 10 分钟检查一次 GitHub Pages 上已经部署的 `.user.js`；页面隐藏、已发现更新或等待重新加载时不会继续请求。
- 用户从其他标签页切回 GitLab 时会立即强制检查一次更新，不受 10 分钟缓存限制。
- 远端 `@version` 高于当前安装版本时，入口上方显示更新 Tag，设置按钮同时显示红点。
- 更新 Tag 支持明确的 Hover / Active 反馈；点击后展开插件并直接进入设置页，由用户确认“安装更新”。
...
1. 点击入口上方的“有更新”Tag，直接进入设置页。
2. 点击“安装更新”，并在新标签页中确认 Tampermonkey 的更新提示。
...
1. 打开脚本面板并点击右上角的分支图标。
2. 在仓库分组下直接查看最近活跃的业务分支。
3. 等待脚本自动完成检查，查看每个活跃业务分支对应 Upstream `dev`、`test`、`blue`、`master` 的状态。
4. 需要重新检查时点击面板右上角的刷新图标；需要提 MR 时点击分支右侧的 MR 图标。

脚本进入分支视图时读取 10 个 Fork 的分支列表，每个 Fork 最多读取 Branches API 返回的前 100 个分支并展示 5 个活跃业务分支。项目之间并行检查，单个项目内部逐分支处理；同一分支最多并行检查 4 个目标环境。
...
进入分支视图时会请求各 Fork 的分支列表、Upstream 环境分支和 compare API，并自动生成合入状态。同一业务分支最多并行检查 4 个目标环境；项目之间并行，单个项目内部逐分支处理。
...
8. 已安装的旧版本读取实际部署脚本的 `@version`，在入口上方显示更新 Tag，并在设置按钮显示红点。
...
11. **更新来源唯一**：更新 Tag 和红点必须比较当前安装版本和已经部署的 `.user.js`，不得根据未部署的源码版本提前提示。
