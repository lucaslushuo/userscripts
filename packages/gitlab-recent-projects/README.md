...
一个适用于 HTTPS GitLab 实例的 userscript。它根据当前用户最近创建的 Merge Request（MR）整理常用项目，并将个人 Fork 与对应 Upstream 放在同一组中，同时提供全局项目搜索和常用项目操作入口。
...
- 设置页可以立即检查更新、打开安装更新页面，以及在安装完成后重新加载 GitLab。
...
3. 安装完成后返回 GitLab，点击“安装后重新加载”。

userscript 不能绕过 Tampermonkey 的安全确认静默替换自身，因此安装确认和页面重新加载是两个明确步骤。
...
- 从 API 返回的项目和 MR URL 必须与当前页面同源，否则会被丢弃。
...
| 自有项目 | `GET /api/v4/projects` | `owned=true`、排除 archived |
| 项目搜索 | `GET /api/v4/projects` | `search=<query>`，可选 `owned=true` |
| 搜索 fallback | `GET /api/v4/search` | `scope=projects`、`search=<query>` |

最近 MR 最多读取 5 页，每页最多 100 条；获得足够的项目组后会提前停止。项目列表请求每类最多读取 100 个项目。这些限制用于控制请求量，不保证覆盖账号可访问的全部历史数据。
...
- 重新打开安装地址检查待安装版本。
- 如果通过脚本设置页安装，确认更新后需要返回 GitLab 并重新加载页面。
